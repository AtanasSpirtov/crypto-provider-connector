...
public class OAuth2Controller {
    ...
    public ResponseEntity<String> getLoginUrl(@PathVariable Provider providerId) {
    ...
    public ResponseEntity<Void> callback(@PathVariable Provider providerId, @RequestParam("code") String code) {
        Optional<OAuth2Provider> provider = providerRegistry.getProvider(providerId);
        if (provider.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
